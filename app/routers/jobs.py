...
async def new_job(request: Request):
    return templates.TemplateResponse(
        ...
        template_context(
            ...
            pasted=request.query_params.get("pasted", ""),
        ),
    )
...
async def create_job(request: Request, db: Session = Depends(get_db)):
    ...
    pasted = str(form.get("pasted") or "").strip()
    if not url and not pasted:
        return RedirectResponse(
            "/jobs/new?error=" + quote("Add a job URL, paste the posting, or both."),
            status_code=303,
        )
    ...
    source_text = pasted or scraped_text
    ...
    if not source_text and not warning:
        warning = "No job text yet. Paste the description on the next screen."
...
async def update_job(request: Request, job_id: int, db: Session = Depends(get_db)):
    ...
    job.company = str(form.get("company") or "").strip()
...
async def download_cv_pdf(request: Request, job_id: int, db: Session = Depends(get_db)):
    ...
    filename = _pdf_filename(cv.contact.full_name or "resume", job.company or job.title, "cv")
    return Response(
        content=pdf,
...
async def download_letter_pdf(request: Request, job_id: int, db: Session = Depends(get_db)):
    ...
    filename = _pdf_filename(cv.contact.full_name or "letter", job.company or job.title, "cover-letter")
    return Response(
        content=pdf,
        media_type="application/pdf",
        headers={"Content-Disposition": f'attachment; filename="{filename}"'},
